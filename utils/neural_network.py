"""
SCRIPT DEFINING NEURAL NETWORK CLASS
"""
...
class NeuralNetwork:
    def __init__(self, optimizer, learning_rate, batch_size, epochs, hidden_layers, activation):
        ...
        # input layer
        self.model.add(InputLayer(input_shape=(300,)))
        # hidden layers
        for layer in hidden_layers:
            self.model.add(Dense(layer, activation))
        # output layer
    ...
    def train(self, X_train, y_train, X_val, y_val, verbose=0, save_model=False, output_path=False):
        """
    ...
    def save_model_details(self, output_path):
        """
        Save parameters and results of the model in a txt file
        """
        # define file path to save model details
        file_path = os.path.join(output_path, "model_details.txt")
        # save information
        with open(file_path, 'w') as f:
            f.write(f"NEURAL NETWORK DETAILS for {embedding_type}\n")
            f.write("----------------------\n\n")
